...
from PyQt5.QtWidgets import QDialog, QTableWidget, QTableWidgetItem, QHBoxLayout, \
    QPushButton, QComboBox, QGridLayout, QLabel, QWidget, QLineEdit, QFileDialog, QMessageBox
from PyQt5 import QtGui, QtCore

import pandas as pd
import os

from src.main.log.DoMatchLogger import *
...
class do_match_gui(QWidget):
    ...
    def init_ui(self):
        ...
        self.setWindowIcon(QtGui.QIcon(SOURCE_DIR + ASSETS_DIR + "/icons/window.png"))
        ...
        self.filter_prev_btn.setIcon(QtGui.QIcon(SOURCE_DIR + ASSETS_DIR + "\icons\previous.png"))
        self.filter_prev_btn.clicked.connect(self.go_prev)
        self.filter_next_btn = QPushButton()
        self.filter_next_btn.setIcon(QtGui.QIcon(SOURCE_DIR + ASSETS_DIR + "\icons\\next.png"))
    ...
    def write_Result_to_excel(self):
        try:
            ...
            logging.info("Prev Matched Read")
            del prev_matched['Unnamed: 0']
            logging.info("Unnamed col deleted")
            ...
            # Type checking before writing data
            logging.info("type checking started")

            outFileWriter = pd.ExcelWriter(self.filePath, engine='xlsxwriter')
            logging.info("Out Excel file opened")
